...
addLayer("s", {
		name: "Swaroop", // This is optional, only used in a few places, If absent it just uses the layer id.
        symbol: "SW", // This appears on the layer's node. Default is the id with the first letter capitalized
        position: 4, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
        startData() { return {
            unlocked: false,
			points: new Decimal(0),
        ...
        color: "#edb3ff",
        requires() { return Decimal.sub(108, hasAchievement("a", 164)?player.c.buyables[11].times(2):0).max(8) }, // Can be a function that takes requirement increases into account
        resource: "Swaroop", // Name of prestige currency
        baseResource: "points", // Name of resource prestige is based on
        baseAmount() {return player.i.points}, // Get the current amount of baseResource
		roundUpCost: true,
        type: "static", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
